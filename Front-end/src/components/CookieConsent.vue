<template>
    <div class="card" v-if="!isAccepted">
        ...
        <div class="buttonContainer">
            <button class="acceptButton" @click="acceptCookies">Allow</button>
            <button class="declineButton" @click="declineCookies">Decline</button>
        </div>
    </div>
</template>
...
export default {
  data() {
    return {
      isAccepted: false,
      // Default preferences for cookies
      preferences: {
        tracking: true,
        abTesting: true,
  ...
  methods: {
    acceptCookies() {
      ...
      // Set cookies for all preferences to true
      Cookies.set('cookieConsent', true, { expires: 365 });
      Cookies.set('tracking', true, { expires: 365 });
      Cookies.set('abTesting', true, { expires: 365 });
      Cookies.set('customAnalytics', true, { expires: 365 });
      document.cookie = "cookiesAccepted=true; max-age=31536000; path=/"; // Expires in 1 year
    ...
    declineCookies() {
      this.isAccepted = true;
      // Optionally, set a cookie to remember the decline choice
      document.cookie = "cookiesAccepted=false; max-age=31536000; path=/"; // Expires in 1 year
    },
    loadAnalytics() {
    ...
    loadABTesting() {
      // Load A/B testing script or set up A/B testing logic
      console.log('A/B testing enabled');
      // Implement your A/B testing logic here
    },
    loadCustomAnalytics() {
      // Load custom analytics script or initialize your marketing tool
  ...
  mounted() {
    ...
    if (cookies) {
      this.isAccepted = cookies.split('=')[1] === 'true';
      // If already accepted, load scripts accordingly
      if (this.isAccepted) {
        this.loadAnalytics();
        this.loadABTesting();
        this.loadCustomAnalytics();
